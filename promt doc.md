\# System Context: FibreGems CRM (Headless Architecture)  
\*\*Goal:\*\* I am building a custom CRM for an ISP channel partner (FibreGems). The frontend is built with HTML/TailwindCSS and hosted externally on GitHub Pages. The backend is a Google Apps Script deployed as a Web App (acting as a REST API), and the database is Google Sheets.

Please act as a Senior Full-Stack Developer to assist me with building, debugging, and expanding this system.

\#\# 1\. UI/UX & Theme Guidelines  
\*   \*\*Design Reference:\*\* https://www.fibregems.com/  
\*   \*\*Styling Engine:\*\* TailwindCSS via CDN.  
\*   \*\*Design Requirements:\*\* Match the color scheme, typography, and component styling (buttons, cards, forms) of the reference link. Ensure a modern, responsive layout optimized for both desktop (Admin Command Centre) and mobile/tablet (Agent Field Capture).

\#\# 2\. Database Schema (Google Sheets)  
The database uses Google Sheets as a relational database. Here are the core tables and exact headers:  
\*   \*\*Users:\*\* \`Agent\_ID\`, \`Full\_Name\`, \`Email\`, \`Password\_Hash\`, \`Salt\`, \`Role\`, \`Status\`, \`Verification\_Token\`, \`OTP\`, \`OTP\_Expiry\`, \`Created\_At\`  
\*   \*\*Daily\_Logs:\*\* \`Log\_ID\`, \`Agent\_ID\`, \`Full\_Name\`, \`Login\_Date\`, \`Login\_Timestamp\`, \`Last\_Active\_Time\`, \`Session\_Token\`  
\*   \*\*CUSTOMERS:\*\* \`Customer\_ID\`, \`Record\_Status\`, \`Created\_DateTime\`, \`Created\_By\_Agent\_ID\`, \`Last\_Updated\_DateTime\`, \`First\_Name\`, \`Surname\`, \`Full\_Name\`, \`Cell\_Number\`, \`Alternate\_Cell\_Number\`, \`Email\`, \`Full\_Address\`, \`Area\_Suburb\`, \`Product\_Package\`, \`Payment\_Type\`, \`Order\_Date\`, \`SADV\_OVR\_Number\`, \`Original\_Agent\_ID\`, \`Current\_Owner\_ID\`, \`Team\_Leader\_ID\`, \`Customer\_Status\`, \`Next\_Action\`, \`Next\_Action\_Date\`, \`Worklist\_Status\`, \`Last\_Contact\_Date\`  
\*   \*\*ORDERS:\*\* \`Order\_ID\`, \`Customer\_ID\`, \`Customer\_Name\`, \`Order\_Date\`, \`SADV\_OVR\_Number\`, \`SADV\_OVK\_Number\`, \`Agent Name\`, \`Order Status\`, \`Next Action\`, \`Action Date\`, \`Import Date\`, \`Source\`  
\*   \*\*PAYMENTS:\*\* \`Payment\_ID\`, \`Customer\_ID\`, \`Payment\_Status\`, \`Promised\_Payment\_Date\`, \`Payment\_Date\`, \`Payment\_Verified\_Date\`  
\*   \*\*ACTIVATIONS:\*\* \`Activation\_ID\`, \`Customer\_ID\`, \`Activation\_Status\`, \`Activation\_Date\`, \`Activation\_Verified\_Date\`  
\*   \*\*ACTIVITY\_LOG:\*\* \`Activity\_ID\`, \`Activity\_DateTime\`, \`Customer\_ID\`, \`Customer\_Name\`, \`Agent\_ID\`, \`Action\_Type\`, \`Contact\_Method\`, \`Outcome\`, \`Customer\_Status\_After\`, \`Promised\_Payment\_Date\`, \`Next\_Action\`, \`Next\_Action\_Date\`, \`Customer\_Escalation\`, \`Escalation\_Type\`, \`Escalation\_Reason\`, \`Notes\`, \`Logged\_By\_Agent\_ID\`, \`Created\_DateTime\`  
\*   \*\*Agility REPORT:\*\* \`created\_da\`, \`completed\_\`, \`channel\_partner\_user\_name\`, \`channel\_partner\_name\`, \`status\`, \`paymentrec\`, \`completeddateday\`, \`vuma\_reach\_premise\_id\`, \`core\_product\_supplier\_name\`, \`contractproductname\`, \`description\`, \`custom\_status\`, \`completion\`, \`customer\`, \`Sync\_Status\`

\#\# 3\. Business Logic & Rules  
1\.  \*\*Authentication:\*\* Zero-footprint volatile sessions. Tokens are held in JS memory.  
2\.  \*\*Duplicate Protection:\*\* Checks \`Cell\_Number\` on capture. Soft Warning (update existing) if the agent owns it; Hard Block if another agent owns it.  
3\.  \*\*Agility Sync Engine:\*\* Admin-triggered bulk processor that reads \`Agility REPORT\`, extracts OVR/OVK strings, maps products ("FTTR-20-10" to "Vuma Reach 20Mbps/10Mbps"), and injects records across \`CUSTOMERS\`, \`ORDERS\`, \`PAYMENTS\`, and \`ACTIVATIONS\` cleanly.

\#\# 4\. Architecture: REST API Connection  
Because the frontend is on GitHub, \`google.script.run\` is deprecated. The frontend communicates with Google Apps Script via standard \`fetch()\` POST requests. The GAS backend relies on a \`doPost(e)\` router to parse JSON payloads containing an \`action\` key and execute the corresponding function.

\*\*Current Task:\*\* I need assistance building out the frontend HTML components based on my design reference, and writing the Javascript \`fetch\` wrappers to interact with my \`doPost\` Apps Script endpoints.

### **Backend: `Code.gs` (REST API Update)**

To host your HTML on GitHub, Google Apps Script must be converted from a bound Web App (`doGet` with `HtmlService`) into a headless REST API (`doPost` returning JSON). Replace your entire `Code.gs` file with this code. Deploy it as a Web App, set "Execute as: Me", and "Who has access: Anyone".

JavaScript  
const SHEET\_ID \= SpreadsheetApp.getActiveSpreadsheet().getId();  
const ss \= SpreadsheetApp.openById(SHEET\_ID);
...
function hashPassword(password, salt) {  
  const rawHash \= Utilities.computeDigest(Utilities.DigestAlgorithm.SHA\_256, password \+ salt, Utilities.Charset.UTF\_8);  
  ...
  for (let i \= 1; i \< data.length; i++) {  
    if (data\[i\]\[2\].toString().toLowerCase() \=== email.toLowerCase()) {  
      ...
      if (hashPassword(password, data\[i\]\[4\]) \=== data\[i\]\[3\]) {  
        ...
        const name \= data\[i\]\[1\];  
        const now \= new Date();  
        ss.getSheetByName('Daily\_Logs').appendRow(\[Utilities.getUuid().slice(0,8), agentId, name, Utilities.formatDate(now, Session.getScriptTimeZone(), "yyyy-MM-dd"), now, now, token\]);  
...
function getSessionUser(token) {  
  ...
  const logs \= ss.getSheetByName('Daily\_Logs').getDataRange().getValues();  
...
function handleAgentLeadUpdate(existingIdOverride, formData, token) {  
  ...
  ss.getSheetByName('ACTIVITY\_LOG').appendRow(\['ACT-' \+ Utilities.getUuid().substring(0, 8).toUpperCase(), timestamp, customerId, \`${formData.firstName} ${formData.surname}\`, session.agentId, existingIdOverride ? "Lead Overwritten" : "Lead Captured", "Web Portal", formData.lastContactOutcome||"", formData.customerStatus||"", formData.promisedPaymentDate || "", formData.nextAction||"", formData.nextActionDate||"", "False", "None", "", "", session.agentId, timestamp\]);  
...
function quickUpdateSalesData(customerId, payload, token) {  
  ...
  ss.getSheetByName('ACTIVITY\_LOG').appendRow(\['ACT-' \+ Utilities.getUuid().substring(0,8).toUpperCase(), new Date(), customerId, custData\[rowIndex-1\]\[7\], session.agentId, "Quick Edit", "", "", payload.status, "", payload.nextAction, payload.nextActionDate, "", "", "", "", session.agentId, new Date()\]);  
  return {success: true};  
}
...
function getAdminDashboard(token) {  
  ...
  const todayStr \= Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd");  
  const activityData \= ss.getSheetByName('ACTIVITY\_LOG').getDataRange().getValues().slice(1);  
...
function runAgilitySync(token) {  
  ...
  const SYNC\_COL\_INDEX \= headers.indexOf('Sync\_Status');  
  const CUSTOMER\_COL\_INDEX \= headers.indexOf('customer');  
  const PRODUCT\_COL\_INDEX \= headers.indexOf('contractproductname');  
  const AGENT\_COL\_INDEX \= headers.indexOf('channel\_partner\_user\_name');  
   
  if (SYNC\_COL\_INDEX \=== \-1) throw new Error("Could not find 'Sync\_Status' column header.");  
   
  const syncStatusArray \= agilitySheet.getRange(1, SYNC\_COL\_INDEX \+ 1, agilityData.length, 1).getValues();  
  ...
  for (let i \= 1; i \< agilityData.length; i++) {  
    ...
    if (match) {  
      ...
      if (customerId) {  
        ...
        let agentName \= '';  
        if (AGENT\_COL\_INDEX \!== \-1 && row\[AGENT\_COL\_INDEX\]) {  
            agentName \= String(row\[AGENT\_COL\_INDEX\]).trim();  
        }  
         
        const isPaid \= \!\!row\[5\];  
        const isActivated \= String(row\[11\] || '').toLowerCase().includes('activated');  
        let fullName \= "Agility Import";  
        if (CUSTOMER\_COL\_INDEX \!== \-1 && row\[CUSTOMER\_COL\_INDEX\]) {  
            fullName \= String(row\[CUSTOMER\_COL\_INDEX\]).trim();  
        }  
        ...
        if (PRODUCT\_COL\_INDEX \!== \-1 && row\[PRODUCT\_COL\_INDEX\]) {  
            rawProduct \= String(row\[PRODUCT\_COL\_INDEX\]).toLowerCase();  
  ...
  agilitySheet.getRange(1, SYNC\_COL\_INDEX \+ 1, syncStatusArray.length, 1).setValues(syncStatusArray);  
  return { success: true, newLeads, updatedLeads };  
}

### **Frontend Connector: JavaScript `fetch()` Example**

Since you are migrating to GitHub, replace all `google.script.run` logic in your frontend Javascript with a standard `fetch` wrapper.

JavaScript  
const API\_URL \= "https://script.google.com/macros/s/YOUR\_DEPLOYMENT\_ID/exec"; // Replace with your Web App URL
  ...
  try {  
    const response \= await fetch(API\_URL, {  
      ...
      body: JSON.stringify({  
        action: action,  
        payload: payload,  
        token: typeof session \!== 'undefined' && session ? session.token : null  
      })  
    });  
...
// Example: How to implement Login on the GitHub HTML  
